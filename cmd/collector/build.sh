...
docker run \
	...
	-e GOOS=linux \
	-e GOARCH=amd64 \
	-ti golang:alpine-1.9 sh -c "cd /go/src/github.com/jaegertracing/jaeger/cmd/collector;go build -a -v -x -o collector-linux main.go"
...
docker tag $IMG_NAME registry.cn-shanghai.aliyuncs.com/goiot/jaeger-collector:latest
docker push registry.cn-shanghai.aliyuncs.com/goiot/jaeger-collector:latest
